...
// Manual trading — completely independent from bots
...
// ─── Manual Trading page ──────────────────────────────────────────────────────
export default function ManualTradingPanelPage() {
  ...
  const { data: allPositions } = useQuery<Position[]>({
    queryKey: ["positions-manual"],
    queryFn: async () =>
      (await api.get("/portfolio/positions?open_only=false&limit=200")).data,
    refetchInterval: 4000,
  });

  const positions = allPositions ?? [];
  // STRICT filter: only positions where bot_id is null/undefined = opened manually by the user
  const openPositions   = positions.filter((p) =>  p.is_open  && (p.bot_id === null || p.bot_id === undefined));
  const closedPositions = positions.filter((p) => !p.is_open  && (p.bot_id === null || p.bot_id === undefined));
  ...
  const closeMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["positions-manual"] });
      qc.invalidateQueries({ queryKey: ["orders-manual"] });
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["orders-manual"] }),
  });
